...
public interface AuthController {
        ...
        @Operation(summary = "Signup into the system", description = "Creates a valid user in the db that returns a jwt token", tags = {
                        "User" })
        ResponseEntity<TokenDTO> signup(@RequestBody CreateAccountDTO userInfoDTO);

        // -------------------- READ --------------------
        @PostMapping(path = "/login", consumes = "application/json")
        @Operation(summary = "Login into the system", description = "Checks your credentials are correct and returns a jwt token if valid", tags = {
                        "Users" })
