...
public interface UserController {
        // -------------------- READ --------------------
        @Operation(summary = "Get all users", description = "Retrieve a list of all users in the system", tags = {
                        "User" })
        ResponseEntity<List<UserDTO>> getAllUsers();

        @Operation(summary = "Get user by username", description = "Retrieve a user by their unique username", tags = {
                        "User" })
        ResponseEntity<UserDTO> getUserByUsername(
                        @Parameter(description = "Username of the user") @PathVariable String username);

        @Operation(summary = "Get user by email", description = "Retrieve a user by their unique email", tags = {
                        "User" })
        ResponseEntity<UserDTO> getUserByEmail(
                        @Parameter(description = "Email of the user") @PathVariable String email);

        @Operation(summary = "Get user by ID", description = "Retrieve a user by their unique ID", tags = { "User" })
        ResponseEntity<UserDTO> getUserById(
                        @Parameter(description = "ID of the user") @PathVariable UUID id);

        @Operation(summary = "Get user permissions", description = "Retrieve the permissions of a specific user by ID", tags = {
                        "User" })
        ResponseEntity<String> getUserPermission(
                        @Parameter(description = "ID of the user") @PathVariable UUID id);

        // -------------------- UPDATE --------------------
        @Operation(summary = "Update user password", description = "Update the password of a specific user by ID", tags = {
                        "User" })
